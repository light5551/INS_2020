...
from gens import gen_cross, gen_h_line, gen_v_line


def random_line(img_size=50):
    p = np.random.random([1])
    if p < 0.5:
        return gen_v_line(img_size)
    else:
        return gen_h_line(img_size)
...
def gen_data(size=500, img_size=50):
    ...
    data_c1 = np.array([gen_cross(img_size) for i in range(c1)])
